...
class Player:
    def __init__(self, app, pos):
        self.app = app
        self.grid_pos = pos
        self.pixel_pos = vec(self.grid_pos[0] * CELL_WIDTH - PLAYER_X_INDENT,
                             self.grid_pos[1] * CELL_HEIGHT + TOP_BUFFER - PLAYER_Y_INDENT)
    ...
    def move2(self):
        if not self.minimax.best_pos:
            self.get_path()
        if self.minimax.best_pos:
            x = int(self.grid_pos[0]) - self.minimax.best_pos[0][0]
            y = int(self.grid_pos[1]) - self.minimax.best_pos[0][1]
            self.dir = vec(-x, -y)
            self.minimax.best_pos.pop(0)

    def get_path(self):
        position = Position(self.app, self.app.map, player_pos(self), self.app.enemies, self.app.heart_number,
                            self.app.picked_hearts, False)
        self.minimax.best_pos = []
        self.minimax.best_score = -10000
        self.minimax.minimax(0, True, position, 1000, -1000, [position.player_position])
        if self.minimax.best_pos and player_pos(self) == self.minimax.best_pos[0]:
            self.minimax.best_pos.pop(0)

        return self.minimax.best_pos

    def move_expectimax(self):
        if not self.expectimax.best_pos:
            self.get_path_expectimax()
        if self.expectimax.best_pos:
            x = int(self.grid_pos[0]) - self.expectimax.best_pos[0][0]
            y = int(self.grid_pos[1]) - self.expectimax.best_pos[0][1]
            self.dir = vec(-x, -y)
            self.expectimax.best_pos.pop(0)

    def get_path_expectimax(self):
        position = Position(self.app, self.app.map, player_pos(self), self.app.enemies, self.app.heart_number,
                            self.app.picked_hearts, False)
        self.expectimax.best_pos = []
        self.expectimax.best_score = -10000
        self.expectimax.expectimax(0, True, position, [position.player_position])
        if self.expectimax.best_pos and player_pos(self) == self.expectimax.best_pos[0]:
            self.expectimax.best_pos.pop(0)
    ...
    '''def update(self):
        ...
        if self.correct_pos():
            if self.stored_direction is not None:
                ...
                if self.app.minimax.best_pos:
                    self.app.minimax.best_pos.pop(0)
...
'''
    ...
    def pick_up_heart(self):
        ...
        flag = False
        if self.app.next_hearth == (x, y):
            flag = True
        self.app.map[x][y] = '0'
        self.app.picked_hearts += 1
        self.app.hearts_pos.remove((x, y))
        if flag:
            self.app.next_hearth = self.app.hearts_pos[random.randrange(0, len(self.app.hearts_pos))]
